...
from .charging_report_renderer import render_charging_report_png
...
def get_first_entry_config(hass: HomeAssistant) -> dict[str, Any] | None:
    """Return the first POM Tesla Report config."""
...
def add_trip_map_point(
    ...
    *,
    ...
    if not points:
        ...
        trip_state["last_map_point"] = [lat, lon]
        trip_state["map_point_count"] = len(points)
        return True

    last = points[-1]
    try:
        last_lat = float(last[0])
        last_lon = float(last[1])
    except Exception:
        points.append([lat, lon])
        trip_state["last_map_point"] = [lat, lon]
        trip_state["map_point_count"] = len(points)
...
def update_trip_map_samples_for_active_states(
    ...
    for state_key in (TRIP_STATE_KEY, MANUAL_TRACKING_STATE_KEY):
        ...
        if not trip_state.get("active"):
            continue
        if add_trip_map_point(trip_state, point, min_movement_meters=min_movement, force=False):
...
def get_vehicle_entity_entries(data: dict[str, Any]) -> list[dict[str, Any]]:
    """Return central Vehicle Entity Manager entries."""
...
def get_configured_entity(data: dict[str, Any], legacy_key: str, role: str, *, usage_key: str | None = None) -> str | None:
    """Return central role entity first, then legacy config fallback."""
    return get_vehicle_role_entity(data, role, usage_key=usage_key) or data.get(legacy_key)
...
def get_extra_ai_context_entities(data: dict[str, Any]) -> list[str]:
    """Return manually added AI context entities."""
...
async def async_telegram_send_message(
    ...
    *,
    ...
    """Send a Telegram text message using built-in mode or HA telegram_bot fallback."""
    ...
    raise ValueError("Neither built-in Telegram nor Home Assistant telegram_bot.send_message is available.")
...
async def async_telegram_send_photo(
    ...
    *,
    ...
    """Send a Telegram image/document using built-in mode or HA telegram_bot fallback."""
    target = str(target or "").strip()
    ...
    if hass.services.has_service("telegram_bot", "send_photo"):
        await hass.services.async_call(
            "telegram_bot",
            "send_photo",
            {"target": target, "file": file_path, "caption": caption},
            blocking=True,
        )
        return

    raise ValueError("Neither built-in Telegram nor Home Assistant telegram_bot.send_photo is available.")
...
async def async_build_ai_vehicle_control_action_with_llm(
    ...
    system_prompt = (
        "You are POM Tesla Report's strict vehicle-control intent router. "
...
async def async_build_contextual_vehicle_suggestion_with_llm(
    ...
    *,
    ...
    capability_json = json.dumps(controls, ensure_ascii=False)
    system_prompt = (
        "You are POM Tesla Report's contextual pending-suggestion detector. "
        "Return ONLY valid JSON. No prose. No markdown. "
...
def build_charging_report_data_from_state(
    ...
    *,
    ...
    for key in ("actual_provider", "actual_price_per_kwh", "actual_total_cost"):
        if key in overrides:
...
async def render_and_send_charging_report(
    ...
    *,
    ...
    png_path = await hass.async_add_executor_job(render_charging_report_png, report_data, output_path)
    ...
    if send_telegram and target:
        ...
        await hass.services.async_call(
            "telegram_bot",
            "send_photo",
            {"target": target, "file": png_path, "caption": caption},
            blocking=True,
        )
...
def provider_price_for_charging_report(hass: HomeAssistant, data: dict[str, Any], provider: str) -> tuple[str, float]:
    """Return display label and TL/kWh price for a selected provider."""
    supercharger_price, zes_price, astor_price = get_charging_tariff_prices(hass, data)
...
def parse_charging_money_value(message: str) -> float | None:
    ...
    `12.49 TL`, `12.49tl/kWh` or even `12.49kwh` all become 12.49.
    """
...
async def async_send_other_charging_price_prompt(hass: HomeAssistant, target: str) -> None:
    """Ask for custom TL/kWh value."""
    await hass.services.async_call(
        "telegram_bot",
        "send_message",
        {
...
async def async_send_other_charging_total_prompt(hass: HomeAssistant, target: str, price: float) -> None:
    """Ask for custom total TL value."""
    await hass.services.async_call(
        "telegram_bot",
        "send_message",
        {
...
async def async_send_other_charging_summary_prompt(hass: HomeAssistant, pending: dict[str, Any], target: str) -> None:
...
async def async_timeout_pending_charging_report_if_needed(hass: HomeAssistant, data: dict[str, Any]) -> None:
    """Send the default report if the user does not answer the charging prompt."""
    pending = get_pending_charging_report_state(hass)
    ...
    if created_ts <= 0:
        return
    if (datetime.now().timestamp() - created_ts) < 300:
        return
...
def get_live_trip_entities(data: dict[str, Any]) -> dict[str, str | None]:
    ...
    """
    return {
        "speed": get_configured_entity(data, CONF_SPEED_ENTITY, VEHICLE_ROLE_SPEED, usage_key="use_report"),
        "shift_state": get_configured_entity(data, CONF_SHIFT_STATE_ENTITY, VEHICLE_ROLE_SHIFT_STATE, usage_key="use_report"),
        "odometer": get_configured_entity(data, CONF_ODOMETER_ENTITY, VEHICLE_ROLE_ODOMETER, usage_key="use_report"),
        "energy_remaining": get_configured_entity(data, CONF_ENERGY_REMAINING_ENTITY, VEHICLE_ROLE_ENERGY_REMAINING, usage_key="use_report"),
        "battery_level": get_configured_entity(data, CONF_BATTERY_LEVEL_ENTITY, VEHICLE_ROLE_BATTERY_LEVEL, usage_key="use_report"),
        "elevation": get_configured_entity(data, CONF_ELEVATION_ENTITY, VEHICLE_ROLE_ELEVATION, usage_key="use_report"),
        "climate": get_configured_entity(data, CONF_CLIMATE_ENTITY, VEHICLE_ROLE_CLIMATE, usage_key="use_report"),
    }
...
async def run_live_trip_test_simulator(
    ...
    zes_price = safe_float(data.get(CONF_ZES_PRICE), 0.0)
    supercharger_price = safe_float(data.get(CONF_SUPERCHARGER_PRICE), 0.0)
    astor_price = safe_float(data.get(CONF_ASTOR_PRICE), 0.0)
...
async def update_live_trip_engine(hass: HomeAssistant, entry_id: str, data: dict[str, Any]) -> None:
    ...
    elevation = get_float_state(hass, entities.get("elevation"), safe_float(state.get("last_elevation"), 0.0))
    ...
    if state.get("active"):
        ...
        delta_seconds = max(0.0, min(120.0, now_ts - last_ts))

        if speed < traffic_threshold:
            state["traffic_seconds"] = safe_float(state.get("traffic_seconds"), 0.0) + delta_seconds
    ...
    average_speed = (trip_km / (duration_seconds / 3600.0)) if duration_seconds > 0 else 0.0
    consumption = (used_kwh / trip_km * 100.0) if trip_km > 0 else 0.0
...
def update_runtime_tracking_fields(
    ...
    """Accumulate runtime metrics such as traffic time, climate usage and elevation."""
    trip_state = get_named_state(hass, state_key)
    ...
    previous_speed = safe_float(
        ...
        get_float_state(hass, get_configured_entity(integration_data, CONF_SPEED_ENTITY, VEHICLE_ROLE_SPEED, usage_key="use_report"), 0.0),
    )
    previous_climate_active = bool(
        trip_state.get("last_climate_active", trip_state.get("climate_active_at_start", False))
    )

    if previous_speed < TRAFFIC_SPEED_THRESHOLD_KMH:
        trip_state["traffic_seconds"] = safe_float(trip_state.get("traffic_seconds"), 0.0) + delta_seconds
    ...
    current_speed = get_float_state(hass, get_configured_entity(integration_data, CONF_SPEED_ENTITY, VEHICLE_ROLE_SPEED, usage_key="use_report"), 0.0)
    current_climate_state = get_state_text(hass, get_configured_entity(integration_data, CONF_CLIMATE_ENTITY, VEHICLE_ROLE_CLIMATE, usage_key="use_report"), "")
    current_climate_active = is_climate_active(current_climate_state)
    current_elevation = get_float_state(hass, get_configured_entity(integration_data, CONF_ELEVATION_ENTITY, VEHICLE_ROLE_ELEVATION, usage_key="use_report"), 0.0)
...
def build_trip_report_data_from_json(
    ...
    astor_price = safe_float(integration_data.get(CONF_ASTOR_PRICE), 0.0)
    ...
    report_data = {
        ...
        "average_speed": source.get("average_speed") or source.get("average_overall_speed"),
    ...
    return render_trip_report_png(report_data, output_path)
...
def build_start_state(
    ...
    start_odometer = get_float_state(hass, get_configured_entity(integration_data, CONF_ODOMETER_ENTITY, VEHICLE_ROLE_ODOMETER, usage_key="use_report"), 0.0)
    start_energy = get_float_state(hass, get_configured_entity(integration_data, CONF_ENERGY_REMAINING_ENTITY, VEHICLE_ROLE_ENERGY_REMAINING, usage_key="use_report"), 0.0)
    start_battery = get_float_state(hass, get_configured_entity(integration_data, CONF_BATTERY_LEVEL_ENTITY, VEHICLE_ROLE_BATTERY_LEVEL, usage_key="use_report"), 0.0)
    start_elevation = get_float_state(hass, get_configured_entity(integration_data, CONF_ELEVATION_ENTITY, VEHICLE_ROLE_ELEVATION, usage_key="use_report"), 0.0)
    start_speed = get_float_state(hass, get_configured_entity(integration_data, CONF_SPEED_ENTITY, VEHICLE_ROLE_SPEED, usage_key="use_report"), 0.0)
    start_shift_state = get_state_text(hass, get_configured_entity(integration_data, CONF_SHIFT_STATE_ENTITY, VEHICLE_ROLE_SHIFT_STATE, usage_key="use_report"), "")
    climate_state = get_state_text(hass, get_configured_entity(integration_data, CONF_CLIMATE_ENTITY, VEHICLE_ROLE_CLIMATE, usage_key="use_report"), "")
...
def build_ai_personality_prompt(data: dict[str, Any]) -> str:
    ...
    personality_map = {
        ...
        AI_PERSONALITY_FUNNY: "Esprili, sıcak ve karakterli konuş. Gerekirse şakalaş, ama verileri yanlış aktarma. Araç komutlarında da aynı genel tonu koru; gereksiz resmileşme.",
...
def build_alert_diagnostics_message(hass: HomeAssistant, data: dict[str, Any]) -> str:
    ...
    battery_entity = get_configured_entity(data, CONF_BATTERY_LEVEL_ENTITY, VEHICLE_ROLE_BATTERY_LEVEL, usage_key="use_report")
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    """Set up POM Tesla Report services."""
    ...
    if static_dir.exists():
        try:
            ...
            _LOGGER.debug("POM Tesla Report static frontend path registered: /%s", DOMAIN)
            ...
            try:
                ...
                _LOGGER.debug("POM Tesla Report legacy static frontend path registered: /%s", DOMAIN)
            except Exception as fallback_err:
                _LOGGER.warning(
                    "Could not register POM Tesla Report frontend path: %s / %s",
                    err,
                    fallback_err,
                )
    ...
    async def async_finish_state_core(
        ...
        *,
        ...
        end_odometer = get_float_state(hass, get_configured_entity(integration_data, CONF_ODOMETER_ENTITY, VEHICLE_ROLE_ODOMETER, usage_key="use_report"), 0.0)
        end_energy = get_float_state(hass, get_configured_entity(integration_data, CONF_ENERGY_REMAINING_ENTITY, VEHICLE_ROLE_ENERGY_REMAINING, usage_key="use_report"), 0.0)
        end_battery = get_float_state(hass, get_configured_entity(integration_data, CONF_BATTERY_LEVEL_ENTITY, VEHICLE_ROLE_BATTERY_LEVEL, usage_key="use_report"), 0.0)
        end_elevation = get_float_state(hass, get_configured_entity(integration_data, CONF_ELEVATION_ENTITY, VEHICLE_ROLE_ELEVATION, usage_key="use_report"), 0.0)
        climate_state = get_state_text(hass, get_configured_entity(integration_data, CONF_CLIMATE_ENTITY, VEHICLE_ROLE_CLIMATE, usage_key="use_report"), "")
        ...
        if is_trip_map_collection_enabled(integration_data):
            ...
            map_data = build_trip_map_render_data(
                trip_state,
                report_data,
                title=f"POM Tesla Report - {get_report_type_label(state_key)}",
            )
        ...
        # Post-trip AI summary removed. The official PNG report already covers trip details.
    ...
    async def async_start_manual_tracking(entry_id: str) -> bool:
        ...
        return await async_start_state_core(
            ...
            title="POM Tesla Report - Manual Tracking",
    ...
    async def async_process_auto_trip_signal(entry_id: str, source_entity: str) -> None:
        ...
        shift_state = get_state_text(hass, get_configured_entity(integration_data, CONF_SHIFT_STATE_ENTITY, VEHICLE_ROLE_SHIFT_STATE, usage_key="use_report"), "")
        speed = get_float_state(hass, get_configured_entity(integration_data, CONF_SPEED_ENTITY, VEHICLE_ROLE_SPEED, usage_key="use_report"), 0.0)
        ...
        if should_auto_finish_trip(shift_state):
            _LOGGER.info(
                "Auto trip finish triggered. shift=%s speed=%s entity=%s",
                shift_state,
                speed,
                source_entity,
            )
    ...
    async def handle_debug_config(call: ServiceCall) -> None:
        """Show selected POM Tesla Report config data."""
        entries = hass.config_entries.async_entries(DOMAIN)
        ...
        await async_create_persistent_notification(
            hass,
            title="POM Tesla Report Debug Config",
            message=message,
            notification_id="pom_tesla_report_debug_config",
        )
        ...
        if send_telegram and telegram_target:
            for chunk in split_telegram_message(message):
                await hass.services.async_call(
                    "telegram_bot",
                    "send_message",
                    {"target": telegram_target, "parse_mode": "plain_text", "message": chunk},
                    blocking=True,
                )
    ...
    async def handle_test_ai_alert(call: ServiceCall) -> None:
        ...
        if send_telegram:
            ...
            override_target = str(call.data.get("telegram_target") or "").strip()
            test_data = dict(data)
            if override_target:
    ...
    async def handle_send_charge_report(call: ServiceCall) -> None:
        ...
        try:
            ...
            await async_create_persistent_notification(
                hass,
                title="POM Tesla Report - Åarj Raporu Hatası",
    ...
    async def handle_start_charge_report_prompt(call: ServiceCall) -> None:
        ...
        try:
            ...
            await async_create_persistent_notification(
                hass,
                title="POM Tesla Report - Åarj Sorusu Hatası",
    ...
    async def handle_generate_test_trip_image(call: ServiceCall) -> None:
        ...
        report_data = {
            ...
            "report_date": datetime.now().strftime("%d.%m.%Y %H:%M"),
            "trip_km": f"{float(call.data['trip_km']):.2f}",
            "duration_text": str(call.data["duration_text"]),
        ...
        await async_create_persistent_notification(
            hass,
            title="POM Tesla Report - Test PNG",
            message=message,
    ...
    async def handle_generate_trip_image_from_json(call: ServiceCall) -> None:
        ...
        await async_create_persistent_notification(
            hass,
            title="POM Tesla Report - JSON PNG",
            message=message,
    ...
    async def handle_debug_trip_state(call: ServiceCall) -> None:
        ...
        await async_create_persistent_notification(
            hass,
            title="POM Tesla Report - Trip State",
            message=message,
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if process_func and watched_entities:
        ...
        _LOGGER.info(
            "POM Tesla Report auto trip listeners registered for entry %s: %s",
            entry.entry_id,
            watched_entities,
        )
    ...
    if map_enabled:
        ...
        _LOGGER.info(
            "POM Tesla Report trip map sampler registered. entry=%s tracker=%s interval=%ss",
            entry.entry_id,
    ...
    _LOGGER.info(
        "POM Tesla Report charging report sampler registered for entry %s",
        entry.entry_id,
    )
    ...
    if get_bool_option(config_data, CONF_LIVE_TRIP_ENABLED, DEFAULT_LIVE_TRIP_ENABLED):
        ...
        _LOGGER.info(
            "POM Tesla Report live trip engine registered for entry %s interval=%ss",
            entry.entry_id,
            live_interval_seconds,
        )

    alerts_enabled = get_bool_option(
        config_data,
        CONF_AI_ALERTS_ENABLED,
        DEFAULT_AI_ALERTS_ENABLED,
    )
    ...
    if alerts_enabled:
        ...
        _LOGGER.info(
            "POM Tesla Report AI Alerts & Watchers registered for entry %s",
            entry.entry_id,
        )

        battery_alert_entity = get_configured_entity(
            config_data,
            CONF_BATTERY_LEVEL_ENTITY,
            VEHICLE_ROLE_BATTERY_LEVEL,
        ...
        if battery_alert_entity and get_bool_option(
            config_data,
            CONF_AI_ALERT_LOW_BATTERY_ENABLED,
            DEFAULT_AI_ALERT_LOW_BATTERY_ENABLED,
        ):
            ...
            unsub_low_battery = async_track_state_change_event(
                hass,
                [battery_alert_entity],
                _handle_low_battery_event,
            )
            entry.async_on_unload(unsub_low_battery)
            _LOGGER.info(
                "POM Tesla Report low battery alert listener registered for entry %s: %s",
                entry.entry_id,
                battery_alert_entity,
            )
    ...
    if listener_enabled:
        ...
        async def _handle_telegram_callback_event(event) -> None:
            ...
            if callback_data.startswith("/pom_charge_other_"):
                ...
                if action == "confirm":
                    ...
                    try:
                        ...
                        await hass.services.async_call(
                            "telegram_bot",
                            "send_message",
                            {"target": target, "parse_mode": "plain_text", "message": f"Åarj raporu oluşturulamadı: {err}"},
                            blocking=True,
                        )
                    return
                ...
                if action == "fix_total":
                    if safe_float(pending.get("actual_price_per_kwh"), 0.0) <= 0:
                        ...
                        await async_send_other_charging_total_prompt(hass, target, safe_float(pending.get("actual_price_per_kwh"), 0.0))
                    return
                ...
                if action == "cancel":
                    ...
                    await hass.services.async_call(
                        "telegram_bot",
                        "send_message",
                        {"target": target, "parse_mode": "plain_text", "message": "Åarj raporu oluşturma iptal edildi."},
                        blocking=True,
                    )
                    return
            ...
            provider_label, price = provider_price_for_charging_report(hass, current_data, selected)
            state_snapshot = pending.get("state") or get_charging_report_state(hass)
            added = safe_float(state_snapshot.get("last_added_kwh"), 0.0)
            manual_overrides = {
                "actual_provider": provider_label,
                "actual_price_per_kwh": price,
                "actual_total_cost": round(added * price, 2),
            }
            try:
                await async_finalize_interactive_charging_report(
                    hass,
        ...
        async def _handle_telegram_text_event(event) -> None:
            ...
            if pending_charge.get("active") and pending_charge.get("step") in {"other_price", "other_total"}:
                ...
                if (not pending_chat_id) or chat_id == pending_chat_id:
                    ...
                    if pending_charge.get("step") == "other_price":
                        ...
                        await async_send_other_charging_total_prompt(hass, target, value)
                        return
                    ...
                    if pending_charge.get("step") == "other_total":
                        ...
                        await async_send_other_charging_summary_prompt(hass, pending_charge, target)
                        return
            ...
            if should_send_last_trip_png(user_message):
                report_info = get_latest_trip_report_info(hass)
                if not report_info:
                    await hass.services.async_call(
                        "telegram_bot",
        ...
        _LOGGER.info(
            "POM Tesla Report Telegram AI listener registered. entry=%s chat_id=%s allowed_user=%s prefix_required=false",
            entry.entry_id,
            listener_chat_id,
        ...
        if is_builtin_telegram_enabled(config_data) and get_bool_option(
            ...
            async def _handle_builtin_telegram_interval(_now) -> None:
                current_data = hass.data.get(DOMAIN, {}).get(entry.entry_id, config_data)
                try:
                    await async_poll_builtin_telegram_updates(hass, entry.entry_id, current_data)
                except Exception as err:
                    _LOGGER.exception("Built-in Telegram polling failed: %s", err)

            unsub_builtin_telegram = async_track_time_interval(
                hass,
                _handle_builtin_telegram_interval,
                timedelta(seconds=poll_interval_seconds),
            )
            entry.async_on_unload(unsub_builtin_telegram)
            hass.async_create_task(async_poll_builtin_telegram_updates(hass, entry.entry_id, config_data))

            _LOGGER.info(
                "POM Tesla built-in Telegram polling registered. entry=%s interval=%ss chat_id=%s",
                entry.entry_id,
...
async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    _LOGGER.info(
        "POM Tesla Report options updated. Reloading entry %s",
        entry.entry_id,
    )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload POM Tesla Report config entry."""
