    ...
    return vol.Schema(
        {
            vol.Optional(
                ...
                default=(
                    ...
                    "If replies are enabled, AI answers and report messages use the same group ID below."
                ),
            ): _multiline_text_selector(),
            ...
            vol.Required(
                ...
                default=bool(
                    str(current.get(CONF_TELEGRAM_TARGET) or current.get(CONF_AI_TELEGRAM_TARGET) or "").strip()
                ),
            ): selector.BooleanSelector(),
    ...
    return vol.Schema(
        {
            vol.Optional(
                ...
                default=(
                    "Select one sample Tesla entity from Tessie or TeslaMate, then use automatic discovery to find AI context entities from the same vehicle. "
                    "Example: choose one battery, climate, lock, or vehicle state entity, then select 'Find AI entities automatically' at the bottom and submit."
                ),
            ): _multiline_text_selector(),
            ...
            vol.Required(UI_VEHICLE_MANAGER_ACTION, default=ACTION_VEHICLE_SAVE_REVIEW): _ai_entity_manager_action_selector(),
        }
    )
        ...
        if user_input is not None:
            ...
            if action == ACTION_VEHICLE_AUTO_SELECT_AI:
                ...
                for entity_id in list(selected_raw) + [
                    ...
                    entity_id = str(entity_id or "").strip()
                    if entity_id and entity_id not in combined_ai_entities:
