...
No entity role mapping definitions were changed in alpha294.

Drive Dashboard speed source clarification:
- The Drive Dashboard uses the existing `dashboard_top_speed` role through the generated `entities.speed` YAML binding.
- This is the same dashboard speed binding used by the main Tesla Dashboard.
- If `entities.speed` is missing because the Drive YAML was not rebuilt or the role is empty, fallback is now `sensor.tesla_speed` first, then `sensor.pom_speed`.

Auto Find and entity role selection logic were not changed.
