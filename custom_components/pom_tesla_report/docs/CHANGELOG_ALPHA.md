...
- Removed the AI settings field for reverse-geocoding/address lookup cache duration from the menu.
- The address lookup cache remains fixed internally at 60 minutes.
- No Live Trip calculations, Telegram pipeline, AI trip comments, dashboard math, entity mappings, or driving logic changed.

## alpha361 – Alerts built-in Telegram listener fix

- Fixes Proactive Automations / AI Alerts low-battery warnings not sending through the built-in POM Telegram bot.
- `send_pom_ai_alert()` now uses `async_telegram_send_message()` so built-in Telegram is the primary path; Home Assistant Telegram remains only an optional fallback.
...
- Fixes the main Tesla dashboard top-area labels when the app language is English.
...
- Adds tap-to-close behavior to the Charge popup/card: tapping the Charge popup body now turns off the popup helper, matching the Live Trip AI popup reader interaction.
- Keeps the existing Charge popup width/position tuning and does not change Charge calculations or entity bindings.

- Adds a tap-to-read popup for the existing Live Trip Comment box: tapping the comment opens a large in-car readable overlay; tapping the popup closes it.
- The existing Live Trip Comment panel layout, calculations, AI interval logic, Telegram/AI report pipeline, candidate maneuver filter, Trip Records, and Charge popup are left unchanged.
- Popup uses large fonts and small detail chips for segment, distance, duration, traffic, consumption, location, elevation, climate, and speed when the data is available.
- Stable card type remains `custom:pom-tesla-trip-report-card`; frontend cache version bumped to `2.2.0-dashboard-alpha358-charge-popup-tap-close`.
- Active panel: `pom-tesla-report-panel-alpha358.js` / `pom-tesla-report-panel-alpha358`.

## alpha356 – Charge popup close button anchored to card edge
- Based on alpha355.
- Fixed the Charge popup close button positioning by removing the fragile fixed/global viewport coordinate calculation.
- The close control now uses the same responsive width/scale rules as the Charge popup panel, so it stays attached to the panel's top-right edge on Tesla small viewport sizes.
- Charge popup width from alpha354/alpha355 is preserved.
- No changes to Live Trip calculations, candidate trip filtering, Telegram, AI, Trip Records, entity mapping, or report math.
- Manifest/panel/frontend version: `2.2.0-dashboard-alpha356-charge-popup-close-inside-align`; active panel: `pom-tesla-report-panel-alpha356.js` / `pom-tesla-report-panel-alpha356`.
...
## alpha294 - Drive switcher cleanup and speed source clarity

- Built on alpha293, which was based on Berkan's last working alpha284(2) package.
- No Kiosk Mode is used. No `kiosk_mode:` YAML block and no external fullscreen dependency were added.
- Clarified Drive Dashboard speed source behavior:
  - The Drive card uses the same generated `entities.speed` binding that comes from Entities > Dashboard role `dashboard_top_speed`.
  - If that binding is missing in the generated YAML, the fallback order is now `sensor.tesla_speed` first and then `sensor.pom_speed`.
- Strengthened Drive Dashboard top-right chrome cleanup:
  - The HA/Drive top-right `Updated just now` / update / refresh chrome is targeted earlier from the app switcher controller and again from the Drive card guard.
  - The footer `Last Update` remains untouched.
- Kept the app-owned body-level `TESLA | DRIVE` switcher.
- Kept the alpha293 early fullscreen/chrome guard behavior and localStorage fullscreen intent mirror.
- No Auto Find, entity mapping roles, Trip Records, Charge Records, AI/Telegram, report generation, or HGS-removed logic was changed.
