...
Current package: alpha294 Drive switcher cleanup and speed source clarity.

Base: alpha293, which itself was based on the last working `pom_tesla_report_dashboard_alpha284_dashboard_switcher_full(2).zip` selected by Berkan. Do not base future work on alpha286-alpha291 fullscreen/shell/kiosk experiments unless explicitly requested.

Active panel: `pom-tesla-report-panel-alpha294.js` / `pom-tesla-report-panel-alpha294`. Manifest version: `2.2.0-dashboard-alpha294-switcher-update-speed-source`.

Important user preference:
- Do not add Kiosk Mode or any external fullscreen/kiosk dependency without asking Berkan first.
- The preferred direction is that fullscreen/navigation behavior works from the integration/application itself.

What changed in alpha294:
- Kiosk Mode remains absent. No `kiosk_mode:` YAML block is generated.
- The app-owned body-level `TESLA | DRIVE` switcher remains.
- Drive Dashboard top-right Home Assistant/update/refresh chrome hiding was strengthened in two places:
  - the switcher controller tries to hide small top-right Drive chrome as early as possible when fullscreen intent is remembered in localStorage;
  - `pom-tesla-drive-dashboard-card.js` repeats the cleanup after the card connects and after HA state hydration.
- Drive card speed fallback was clarified. The primary value is still the generated `entities.speed` binding from Entities > Dashboard role `dashboard_top_speed`; if absent, fallback is `sensor.tesla_speed`, then `sensor.pom_speed`.
- Footer `Last Update` remains visible.

Important limitation:
- Real browser Fullscreen API cannot run before page load without user activation. The app can only hide Home Assistant chrome after the frontend/dashboard JS starts executing.
