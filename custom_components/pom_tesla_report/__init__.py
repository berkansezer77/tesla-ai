...
async def async_send_other_charging_summary_prompt(hass: HomeAssistant, pending: dict[str, Any], target: str) -> None:
    ...
    await hass.services.async_call(
        ...
        "send_message",
        {
            ...
            "inline_keyboard": [
                "âœ… Onayla:/pom_charge_other_confirm, ✏️ Fiyatı düzelt:/pom_charge_other_fix_price",
                "✏️ Toplamı düzelt:/pom_charge_other_fix_total, 🔄 Baştan başla:/pom_charge_other_restart",
                "âŒ İptal:/pom_charge_other_cancel",
            ],
        },
        blocking=True,
    )
...
async def async_finalize_interactive_charging_report(
    ...
    *,
    manual_overrides: dict[str, Any] | None = None,
    message_prefix: str = "Åarj seansı tamamlandı.",
) -> tuple[str, dict[str, Any]]:
            ...
            if report_mode == CHARGING_REPORT_MODE_PROMPT:
                ...
                if prompted:
                    ...
                    png_path, _report_data = await render_and_send_charging_report(
                        ...
                        state=state,
                        send_telegram=True,
                        message_prefix="Åarj seansı tamamlandı.",
                    )
        ...
        async def _handle_telegram_callback_event(event) -> None:
                ...
                if action == "confirm":
                    ...
                    try:
                        await async_finalize_interactive_charging_report(
                            ...
                            pending,
                            manual_overrides=manual_overrides,
                            message_prefix="Åarj raporu onayladığın gerçek maliyetle oluşturuldu.",
                        )
                    except Exception as err:
            ...
            if selected == "skip":
                try:
                    await async_finalize_interactive_charging_report(
                        ...
                        current_data,
                        pending,
                        message_prefix="Åarj raporu kayıtlı tarifelerle oluşturuldu.",
                    )
                except Exception as err:
            ...
            try:
                await async_finalize_interactive_charging_report(
                    ...
                    pending,
                    manual_overrides=manual_overrides,
                    message_prefix=f"Åarj raporu {provider_label} tarifesiyle oluşturuldu.",
                )
            except Exception as err:
