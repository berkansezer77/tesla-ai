...
def update_trip_map_samples_for_active_states(
    ...
    if not get_bool_option(integration_data, CONF_TRIP_MAP_ENABLED, DEFAULT_TRIP_MAP_ENABLED):
        return
...
def _get_vehicle_state_text(hass: HomeAssistant, data: dict[str, Any]) -> str:
    entity_id = (
...
def build_direct_vehicle_status_answer(hass: HomeAssistant, data: dict[str, Any], message: str) -> str | None:
    ...
    if _has_any_phrase(n, ["uyanik", "uyuyor", "online", "cevrimici", "sleep", "asleep", "awake"]):
        return _get_vehicle_state_text(hass, data)
...
def build_contextual_vehicle_suggestion_without_llm(
    ...
    *,
    ...
    if ans and (
        _has_any_phrase(ans, ["klimayi ac", "klima ac", "serinleteyim", "sogutayim", "isitayim", "sicakligi yukselt"])
...
async def async_build_vehicle_location_answer(hass: HomeAssistant, data: dict[str, Any]) -> str:
...
def provider_price_for_charging_report(data: dict[str, Any], provider: str) -> tuple[str, float]:
...
def build_final_ai_system_prompt(data: dict[str, Any]) -> str:
    """Build final system prompt from app-managed prompt and optional custom notes."""
    generated = build_ai_personality_prompt(data)
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    ...
    async def async_start_state_core(
        ...
        *,
        ...
        if get_bool_option(integration_data, CONF_TRIP_MAP_ENABLED, DEFAULT_TRIP_MAP_ENABLED):
            tracker_entity = get_vehicle_role_entity(integration_data, VEHICLE_ROLE_LOCATION_TRACKER, usage_key="use_map") or integration_data.get(CONF_TRIP_MAP_TRACKER_ENTITY, DEFAULT_TRIP_MAP_TRACKER_ENTITY)
    ...
    async def async_finish_state_core(
        ...
        *,
        ...
        if map_png_path and get_bool_option(integration_data, CONF_TRIP_MAP_ENABLED, DEFAULT_TRIP_MAP_ENABLED) and get_bool_option(
            integration_data,
            CONF_TRIP_MAP_SEND_SEPARATE_PNG,
            DEFAULT_TRIP_MAP_SEND_SEPARATE_PNG,
        ):
    ...
    async def handle_generate_test_trip_image(call: ServiceCall) -> None:
        ...
        supercharger_price = float(data.get(CONF_SUPERCHARGER_PRICE, 0.0))
        zes_price = float(data.get(CONF_ZES_PRICE, 0.0))
        astor_price = float(data.get(CONF_ASTOR_PRICE, 0.0))
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up POM Tesla Report from a config entry."""
    hass.data.setdefault(DOMAIN, {})
    ...
    if listener_enabled:
        ...
        async def _handle_telegram_callback_event(event) -> None:
                ...
                async def _edit_or_send_result(result_message: str) -> None:
                    ...
                    if target:
                        await hass.services.async_call(
                            "telegram_bot",
                            "send_message",
                            {"target": target, "parse_mode": "plain_text", "message": result_message},
                            blocking=True,
                        )
            ...
            provider_label, price = provider_price_for_charging_report(current_data, selected)
            state_snapshot = pending.get("state") or get_charging_report_state(hass)
        ...
        async def _handle_telegram_text_event(event) -> None:
            ...
            if any(candidate and should_send_vehicle_location_answer(candidate) for candidate in command_candidates):
                try:
                    location_message = await async_build_vehicle_location_answer(hass, current_data)
                    await hass.services.async_call(
                        "telegram_bot",
                        "send_message",
                        {
            ...
            direct_status_answer = build_direct_vehicle_status_answer(hass, current_data, user_message)
