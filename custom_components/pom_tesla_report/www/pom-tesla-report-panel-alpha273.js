class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha273-health-trend-safe";
  ...
  _t(key) {
    const dict = {
      tr: {
        ...
        settingsBackupSub: "Ayarları dışa veya içe aktar. Bu alan günlük kullanım için ana ekranda kalır.",
        none: "Yok",
      ...
      en: {
        ...
        settingsBackupSub: "Export or import settings. This stays on the main General screen for daily use.",
        none: "None",
  ...
  _normalizeSettingsPayload(payload) {
    ...
    return {
      ...
      ai_settings: {
        ...(payload?.ai_settings || {}),
        reverse_geocoding_cache_minutes: Math.max(5, Number(payload?.ai_settings?.reverse_geocoding_cache_minutes ?? 5)),
      },
      dashboard_settings: {
  ...
  async _loadHealth(showStatus = false) {
    ...
    try {
      ...
      if (merged && showStatus) {
        ...
        this._pushPomEvent("success", this._t("healthLoadedEvent"), payload?.health || payload, "health");
      }
      const warnings = payload?.health?.warnings || payload?.general_settings?.health?.warnings || [];
      if (Array.isArray(warnings) && warnings.length) {
        this._pushPomEvent("warning", this._t("healthWarnings"), { warnings }, "health");
      }
      this._render();
    } catch (err) {
  ...
  async _loadSystemLogs(showStatus = true) {
    ...
    try {
      ...
      if (available && lineCount > 0) {
        ...
        this._pushPomEvent("success", this._t("haLogsLoadedEvent"), { lines: lineCount, path: this._systemLogsPayload?.path || "" }, "ha_logs");
      } else {
  ...
  async _runSystemControlCheck() {
    ...
    try {
      ...
      this._error = this._formatError(err);
      this._pushPomEvent("error", "System control check failed", { error: this._error }, "system_check");
    } finally {
      this._loading = false;
      this._render();
    }
  ...
  _supportReportPayload() {
    ...
    return {
      ...
      ha_log_tail: {
        ...
        lines_returned: this._systemLogsPayload?.lines_returned || 0,
        lines: Array.isArray(this._systemLogsPayload?.lines) ? this._systemLogsPayload.lines.slice(0, 500) : [],
      },
      status: this._status || "",
      error: this._error || "",
    };
...
.ai-role-meta-line { color:#fca5a5; }
  ...
  _renderGeneralSettings() {
    ...
    return `
      <div class="settings-grid wide">
        ...
        <section class="card">
          <div class="card-h"><div><h2>${this._t("settingsBackup")}</h2><div class="hint">${this._t("settingsBackupSub")}</div></div></div>
          <div class="body">
      ...
      <div class="settings-grid wide">
        <section class="card accent">
          ...
          <div class="body">
            ...
            <div class="field-grid">
              <div><label>${this._t("reverseGeocodingCacheMinutes")}</label><input id="ai_settings_reverse_geocoding_cache_minutes" inputmode="numeric" value="${this._esc(Math.max(5, Number(ai.reverse_geocoding_cache_minutes ?? 5)))}" /><div class="note">${this._t("minutesShort")} · min 5</div></div>
            </div>
  ...
  _readAISettingsForm() {
    ...
    return {
      ...
      reverse_geocoding_cache_minutes: Math.max(5, number("ai_settings_reverse_geocoding_cache_minutes", current.reverse_geocoding_cache_minutes ?? 5)),
  ...
  _buildSystemControlItems() {
    ...
    return [
      ...
      },
      {
        ...
        status: live.active ? "active" : (warnings.length ? "warning" : "ready"),
        detail: live.active ? this._t("serviceLiveTripActive") : (warnings[0] || this._t("serviceLiveTripReady")),
      },
      {
        key: "trip_records",
        icon: "📘",
        ...
        detail: tripRecordsOk === false ? this._t("serviceWriteError") : (tripRecordsOk === true ? this._t("serviceWritable") : this._t("serviceRefreshToTest")),
      },
      {
        key: "charge_records",
        icon: "🔌",
        ...
        detail: chargingRecordsOk === false ? this._t("serviceWriteError") : (chargingRecordsOk === true ? this._t("serviceWritable") : this._t("serviceRefreshToTest")),
      },
      {
        key: "telegram",
        icon: "✈️",
...
if (!customElements.get("pom-tesla-report-panel-alpha273")) {
  customElements.define("pom-tesla-report-panel-alpha273", PomTeslaReportPanel);
}
