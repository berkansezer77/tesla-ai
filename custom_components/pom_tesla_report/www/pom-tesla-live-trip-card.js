class PomTeslaTripReportCard extends HTMLElement {
  static get version() { return '1.8.10-live-alias'; }
  constructor() {
    super();
  ...
  _storageKey() {
    return `pom-tesla-live-trip-card:${this._config.storage_key || 'default'}`;
  }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .traffic-model-line {
          padding: clamp(10px, 1.4cqw, 16px) clamp(16px, 2.2cqw, 34px);
      ...
      <ha-card>
        <div class="wrap">
          ...
          ${trafficRefSpeed > 0 ? `
            <div class="traffic-model-line">
              Trafik modeli: gecikme ${trafficDelayText || '—'} · yoğunluk +%${this._formatNumber(trafficPercent, 0, '0')} · ref ${this._formatNumber(trafficRefSpeed, 0, '0')} km/sa${trafficType ? ` · ${trafficType}` : ''}${trafficImpact ? ` · etki ${trafficImpact}` : ''}
            </div>
          ` : ''}
          <div class="bottom">
            <div class="box">
...
if (!customElements.get('pom-tesla-live-trip-card')) {
  customElements.define('pom-tesla-live-trip-card', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-live-trip-card-editor')) {
  customElements.define('pom-tesla-live-trip-card-editor', PomTeslaTripReportCardEditor);
}
PomTeslaTripReportCard.getConfigElement = function() {
  return document.createElement('pom-tesla-live-trip-card-editor');
};

window.customCards = window.customCards || [];
window.customCards.push({
  type: 'pom-tesla-live-trip-card',
