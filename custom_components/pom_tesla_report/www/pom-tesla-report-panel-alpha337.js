class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha337-live-trip-ai-10km-scheduler-fix";
  ...
  async _runLiveTripAiCommentTest() {
    ...
    this._loading = true;
    this._error = "";
    this._showWaitModal(this._lang === "en" ? "Preparing AI comment" : "AI yorumu hazırlanıyor", this._lang === "en" ? "Please wait until the test comment is generated." : "Test yorumu hazırlanırken lütfen bekleyin.");
    this._render();
    try {
      this._settingsData = this._applySettingsSaveResponse(await this._hass.callApi("POST", "pom_tesla_report/settings", { trip_reports }), { trip_reports });
      const result = await this._hass.callApi("POST", "pom_tesla_report/live_trip_ai_test", { segment_km: segmentKm });
      this._status = result?.message || this._t("liveTripAiCommentTested");
...
if (!customElements.get("pom-tesla-report-panel-alpha337")) {
  customElements.define("pom-tesla-report-panel-alpha337", PomTeslaReportPanel);
}
