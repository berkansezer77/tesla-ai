class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha153-youtube-jsmpeg-progressive-fix";
    ...
    return `
      <div class="dashboard-upload-card youtube-bg-card">
        ...
        <div class="field-grid single-col">
          <div><label>${this._t("dashboardYoutubeVideo")}</label><input id="dashboard_youtube_driving_bg_video" value="${this._esc(cfg.video || "")}" placeholder="https://www.youtube.com/watch?v=..."></div>
          <div><label>${this._t("dashboardYoutubeStartSeconds")}</label><input id="dashboard_youtube_driving_bg_start_seconds" type="number" min="0" step="1" value="${this._esc(String(cfg.start_seconds ?? 0))}"></div>
        </div>
        <div class="note">YouTube iframe, sürüş durumunda driving background görselini görsel olarak bypass eder. Kapatınca eski driving background aynen çalışır.</div>
      </div>
    `;
  }
...
if (!customElements.get("pom-tesla-report-panel")) {
  customElements.define("pom-tesla-report-panel-alpha153", PomTeslaReportPanel);
}
