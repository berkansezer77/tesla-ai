class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha256-trip-record-map-ai-recovery";
  ...
  async _loadHealth(showStatus = false) {
    ...
    try {
      ...
      if (this._mergeHealthPayload(payload) && showStatus) {
  ...
  async _loadLiveTripDebug(showStatus = false) {
    ...
    try {
      ...
      if (showStatus) this._error = this._formatError(err);
    } finally {
  ...
  async _loadSystemLogs(showStatus = true) {
    ...
    try {
      this._systemLogsPayload = await this._hass.callApi("GET", "pom_tesla_report/system_logs");
...
.ai-role-meta-line { color:#fca5a5; }
  ...
  _renderSystemStatusCenter() {
    const diag = this._liveTripDiagnostics || {};
    const states = diag.entity_states || {};
    ...
    const liveStatus = live.active ? "active" : (live.finishing ? "finishing" : (live.restored_from_disk ? "restored" : "idle"));
    const lastReport = live.last_report_saved_at || live.last_finished_at || live.persisted_at || "—";
    return `
      <section class="card system-status-card">
        <div class="card-h"><div><h2>${this._t("systemStatusCenter")}</h2><div class="hint">${this._t("systemStatusCenterSub")}</div></div><button id="refreshGeneralLiveTripDebugBtn" class="secondary">${this._t("refreshSystemStatus")}</button></div>
        <div class="body">
          <div class="status-center-grid">
            <div><span>${this._t("statusLiveTrip")}</span><b>${this._esc(liveStatus)}</b></div>
            <div><span>${this._t("statusSpeed")}</span><b>${this._esc(stateValue("speed"))}</b></div>
            <div><span>${this._t("statusShift")}</span><b>${this._esc(stateValue("shift_state"))}</b></div>
            <div><span>${this._t("statusOdometer")}</span><b>${this._esc(stateValue("odometer"))}</b></div>
            <div><span>${this._t("statusLastReport")}</span><b>${this._esc(String(lastReport).slice(0, 24))}</b></div>
            <div><span>${this._t("statusWarnings")}</span><b>${this._esc(String(warnings.length))}</b></div>
          </div>
          ${warnings.length ? `<div class="status-warning-list">${warnings.map((w) => `<div>${this._esc(w)}</div>`).join("")}</div>` : `<div class="note">${this._lang === "en" ? "No live-trip warning in the latest diagnostic snapshot." : "Son diagnostic görüntüsünde Live Trip uyarısı yok."}</div>`}
          ${this._renderSystemLogsPanel()}
        </div>
      </section>
    `;
  }
...
if (!customElements.get("pom-tesla-report-panel-alpha256")) {
  customElements.define("pom-tesla-report-panel-alpha256", PomTeslaReportPanel);
}
