class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha139-unique-element";
    ...
    this._chargeFilter = "current";
    this._tripFilter = "current";
  ...
  set hass(hass) {
    this._hass = hass;
    if ((!this._chargeData || !this._tripData || !this._settingsData) && !this._loading) {
      this._loadAll();
    }
    // Home Assistant calls the hass setter very frequently. Do not re-render
    // the panel on every hass update after the initial API load, otherwise
    // active input fields such as the charging station editor are rebuilt while
    // the user is typing and their draft text appears to clear itself. Panel
    // actions call _render() explicitly when the UI really needs to change.
  }
  ...
  get _data() {
    ...
    return this._activeTab === "trip" ? this._tripData : this._chargeData;
  }
  ...
  _t(key) {
    const dict = {
      tr: {
        ...
        builtInPricesSub: "Görsel raporlardaki maliyet kartları artık buradaki ilk üç istasyon presetini kullanır.",
        ...
        dashboardYoutubeDrivingTitle: "YouTube Driving Background",
        ...
        dashboardYoutubeDrivingTitle: "YouTube Driving Background",
        dashboardYoutubeDrivingSub: "Araç D/drive durumundayken normal driving background yerine YouTube iframe gösterir. Deneme amaçlıdır.",
        dashboardYoutubeDrivingEnabled: "YouTube driving background aktif",
        dashboardYoutubeVideo: "YouTube video URL / ID",
        ...
        clearAllAiConfirm: "Tüm AI entity seçimleri temizlensin mi?",
        ...
        liveTripFinishDelaySeconds: "Sürüş durduktan sonra bitirme gecikmesi",
      ...
      en: {
        ...
        builtInPricesSub: "Visual report cost cards now use the first three station presets below.",
        ...
        clearAllAiConfirm: "Clear all AI entity selections?",
        ...
        liveTripFinishDelaySeconds: "Finish delay after driving stops",
  ...
  _formatError(err) {
    ...
    if (err.message) {
      ...
      try { return JSON.stringify(err.message); } catch (_e) { return String(err.message); }
    }
  ...
  _normalizeSettingsPayload(payload) {
    ...
    return {
      ...
      telegram: {
        ...
        builtin_telegram_poll_enabled: Boolean(payload?.telegram?.builtin_telegram_poll_enabled ?? true),
  ...
  async _copyDebugInfo() {
    const text = JSON.stringify(this._debugSnapshot(), null, 2);
    let copied = false;
    let method = "";
    try {
  ...
  async _loadAll() {
    ...
    try {
      ...
      const [chargeResult, tripResult, settingsResult, telegramDiagResult, aiDiagResult] = await Promise.allSettled([
        this._hass.callApi("GET", "pom_tesla_report/charge_records"),
        this._hass.callApi("GET", "pom_tesla_report/trip_records"),
        this._hass.callApi("GET", "pom_tesla_report/settings"),
        this._hass.callApi("GET", "pom_tesla_report/telegram_test"),
        this._hass.callApi("GET", "pom_tesla_report/ai_test"),
      ]);
      [
        ["GET charge_records", chargeResult],
        ["GET trip_records", tripResult],
        ["GET settings", settingsResult],
        ["GET telegram_test", telegramDiagResult],
        ["GET ai_test", aiDiagResult],
      ].forEach(([name, result]) => this._recordApiResult(name, result, apiStartedAt));
      const errors = [];
      if (chargeResult.status === "fulfilled") {
        this._chargeData = this._normalizeApiPayload(chargeResult.value);
      } else {
        errors.push(`Charge API: ${this._formatError(chargeResult.reason)}`);
        this._chargeData = this._chargeData || { records: [], summary: {}, language: this._lang, currency: "" };
      }
      if (tripResult.status === "fulfilled") {
        this._tripData = this._normalizeApiPayload(tripResult.value, this._chargeData?.currency || "");
      } else {
        errors.push(`Trip API: ${this._formatError(tripResult.reason)}`);
        this._tripData = this._tripData || { records: [], summary: {}, language: this._chargeData?.language || this._lang, currency: this._chargeData?.currency || "" };
      }
      if (settingsResult.status === "fulfilled") {
        this._settingsData = this._normalizeSettingsPayload(settingsResult.value);
      } else {
        errors.push(`Settings API: ${this._formatError(settingsResult.reason)}`);
        this._settingsData = this._settingsData || this._normalizeSettingsPayload({ language: this._chargeData?.language || this._lang, currency: this._chargeData?.currency || "TL" });
      }
      if (telegramDiagResult.status === "fulfilled") {
        this._telegramDiagnostics = telegramDiagResult.value || { logs: [], service_state: {} };
      } else {
        this._telegramDiagnostics = this._telegramDiagnostics || { logs: [], service_state: {} };
      }
      if (aiDiagResult.status === "fulfilled") {
        this._aiDiagnostics = aiDiagResult.value || { logs: [], service_state: {} };
      } else {
        this._aiDiagnostics = this._aiDiagnostics || { logs: [], service_state: {} };
      }
      this._error = errors.join(" | ");
      this._status = errors.length ? "" : this._t("loaded");
      this._pushDebugEvent(errors.length ? "warning" : "success", errors.length ? "Initial load completed with errors" : "Initial load completed", { errors });
  ...
  _chargeRecords() {
    const records = [...(this._chargeData?.records || [])];
    if (this._chargeFilter === "current") {
      const key = this._chargeData?.summary?.month_key || new Date().toISOString().slice(0, 7);
      return records.filter((r) => r.month_key === key);
    }
    return records;
  }

  _tripRecords() {
    const records = [...(this._tripData?.records || [])];
    if (this._tripFilter === "current") {
      const key = this._tripData?.summary?.month_key || new Date().toISOString().slice(0, 7);
      return records.filter((r) => r.month_key === key);
    }
    return records;
  }
  ...
  _readTripSettingsForm() {
    ...
    return {
      ...
      live_trip_finish_delay_seconds: number("trip_live_trip_finish_delay_seconds", current.live_trip_finish_delay_seconds ?? 90),
  ...
  _groupAIRolesByCategory(roles) {
    const order = ["dashboard_top", "dashboard_sidebar", "dashboard_bottom", "dashboard_map", "dashboard_charge_popup", "dashboard_person", "dashboard_custom_home", "vehicle_controls", "sensors", "diagnostics", "teslamate", "other"];
  ...
  async _saveAIEntityManager() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { ai_entity_manager }));
      this._aiEntityDraft = null;
  ...
  async _saveReportEntityManager() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { report_entity_manager }));
      this._reportEntityDraft = null;
  ...
  async _saveDashboardEntityManager() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { dashboard_entity_manager }));
  ...
  async _saveSettings() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { charging }));
  ...
  async _saveTelegramSettings() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { telegram }));
  ...
  async _saveTripSettings() {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { trip_reports }));
  ...
  _styles() {
    return `
      ...
      .module-card { padding:16px; border:1px solid #273244; border-radius:18px; background:#0f172a; }
      .module-card b { display:block; margin-bottom:6px; }
      .module-card.active { border-color:#38bdf8; background:linear-gradient(135deg, rgba(14,165,233,.18), rgba(15,23,42,.92)); }
      ...
      .station-row { display:flex; justify-content:space-between; gap:14px; align-items:center; padding:14px; background:#101720; border:1px solid #253143; border-radius:16px; cursor:pointer; }
      ...
      .dashboard-upload-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:14px; margin-top:14px; }
      .dashboard-upload-card { border:1px solid rgba(56,189,248,.16); border-radius:18px; background:rgba(15,23,42,.55); padding:14px; display:flex; flex-direction:column; gap:12px; }
      ...
      .dashboard-upload-actions { display:flex; flex-wrap:wrap; gap:8px; align-items:center; }
  ...
  _renderChargeSummary() {
    const s = this._chargeData?.summary || {};
    ...
    return `
      <div class="summary">
        <div class="stat"><span class="muted">${this._t("count")}</span><b>${s.count || 0}</b></div>
  ...
  _renderTripSummary() {
    const s = this._tripData?.summary || {};
    return `
      <div class="summary">
        <div class="stat"><span class="muted">${this._t("count")}</span><b>${s.count || 0}</b></div>
  ...
  _renderStationList() {
    ...
    return `<div class="station-list">
      ${stations.map((s, index) => `
        <div class="station-row ${index === this._selectedStationIndex ? "selected" : ""}" data-station-index="${index}">
          <div><b>${this._esc(s.name)}</b><div class="muted">${this._esc(s.currency)} / kWh</div></div>
  ...
  _renderSettingsNav() {
    ...
    return `
      <div class="settings-nav">
        ${items.map(([id, label]) => `<button id="settingsNav_${id}" data-settings-tab="${this._esc(id)}" class="${this._activeSettingsTab === id ? "active" : ""}">${this._t(label)}</button>`).join("")}
      </div>
    `;
  }
      ...
      <div class="settings-grid wide">
          ...
          <div class="body">
            ...
            ${general.debug_enabled ? `
              <div class="debug-panel">
                <div class="debug-head"><b>${this._t("debugDiagnosticsDetails")}</b><button id="copyDebugInfoBtn" class="secondary">${this._t("debugCopy")}</button></div>
                <div class="debug-grid">
                  <div><span>${this._t("debugRuntime")}</span><pre>${this._esc(JSON.stringify({
                    active_tab: this._activeTab,
                    active_settings_tab: this._activeSettingsTab,
                    active_dashboard_section: this._dashboardSettingsSection,
                    active_trip_section: this._tripSettingsSection,
                    language: debugPayload.language,
                    currency: this._currency,
                    status: debugPayload.status,
                    error: debugPayload.error,
                  }, null, 2))}</pre></div>
                  <div><span>${this._t("debugApiResults")}</span><pre>${this._esc(JSON.stringify((this._lastApiResults || []).slice(0, 8), null, 2))}</pre></div>
                  <div><span>${this._t("debugRecentEvents")}</span><pre>${this._esc(JSON.stringify((this._debugEvents || []).slice(0, 10), null, 2))}</pre></div>
                  <div><span>${this._t("systemSummary")}</span><pre>${this._esc(JSON.stringify({
                    resource_summary: summary || {},
                    entity_store_audit: general.entity_store_audit || {},
                    config_entry: Boolean(system.has_config_entry),
                    telegram_service_state: this._telegramDiagnostics?.service_state || {},
                  ...
                  <div><span>${this._t("debugFrontend")}</span><pre>${this._esc(JSON.stringify({
                    url: window.location?.href || "",
                    user_agent: navigator.userAgent || "",
                    platform: navigator.platform || "",
                    clipboard_api: Boolean(navigator.clipboard?.writeText),
        ...
        <section class="card">
          ...
          <div class="body">
            ...
            <div class="section-title">${this._t("entityStoreAudit")}</div>
            <div class="note">${this._t("entityStoreAuditSub")}</div>
            <pre class="audit-pre">${this._esc(JSON.stringify(general.entity_store_audit || {}, null, 2))}</pre>
            <div class="actions"><button id="migratePanelStoresBtn" class="secondary">${this._t("migratePanelStores")}</button></div>
  ...
  async _migratePanelEntityStores() {
    ...
    try {
      ...
      this._pushDebugEvent("success", "Panel entity stores migrated", response?.migration || this._settingsData?.migration || {});
    } catch (err) {
      this._error = this._formatError(err);
      ...
      <div class="settings-grid">
        ...
        <section class="card">
          ...
          <div class="body">
            ...
            <div class="note">${this._lang === "en" ? "Saved stations appear in the station preset list on the left. Click a station there to edit it here." : "Kaydedilen istasyonlar soldaki istasyon preset listesinde görünür. Düzenlemek için soldaki istasyona bas."}</div>
          </div>
        </section>
      </div>
    `;
      ...
      <div class="settings-grid wide ai-manager-grid ai-manager-stacked compact-ai-manager">
        ...
        <section class="card ai-roles-card compact-ai-card">
          ...
          <div class="body ai-role-list compact">
            ...
            <div class="actions sticky-actions"><button id="saveDashboardEntityManagerBtnBottom">${this._t("saveDashboardEntities")}</button><button id="autoFindDashboardEntitiesBtnBottom" class="secondary">${this._t("autoFindEntities")}</button></div>
          </div>
        </section>
      </div>
    `;
      ...
      <div class="settings-grid wide ai-manager-grid ai-manager-stacked compact-ai-manager">
        ...
        <section class="card ai-roles-card compact-ai-card">
          ...
          <div class="body ai-role-list compact">
            ...
            <div class="actions sticky-actions"><button id="saveAIEntityManagerBtnBottom">${this._t("saveAiEntities")}</button><button id="autoFindAIEntitiesBtnBottom" class="secondary">${this._t("autoFindEntities")}</button></div>
          </div>
        </section>
      </div>
    `;
        ...
        <section class="card">
          ...
          <div class="body">
            <div class="automation-grid">
              ...
              ${this._automationCard("ai_settings_alert_post_trip_summary_enabled", ai.ai_alert_post_trip_summary_enabled, this._t("aiAlertPostTripSummaryEnabled"))}
  ...
  _readAISettingsForm() {
    ...
    return {
      ...
      ai_alert_post_trip_summary_enabled: checked("ai_settings_alert_post_trip_summary_enabled", current.ai_alert_post_trip_summary_enabled),
  ...
  async _saveAISettings(statusKey = "aiSettingsSaved") {
    ...
    this._render();
    try {
      this._settingsData = this._normalizeSettingsPayload(await this._hass.callApi("POST", "pom_tesla_report/settings", { ai_settings }));
  ...
  _renderTripSettings() {
    ...
    const liveTripContent = `
      ...
      <div class="field-grid">
        ...
        <div><label>${this._t("liveTripUpdateIntervalSeconds")}</label><input id="trip_live_trip_update_interval_seconds" inputmode="numeric" value="${this._esc(trip.live_trip_update_interval_seconds ?? 5)}" /><div class="note">${this._t("secondsShort")}</div></div>
        ...
        <div><label>${this._t("liveTripFinishDelaySeconds")}</label><input id="trip_live_trip_finish_delay_seconds" inputmode="numeric" value="${this._esc(trip.live_trip_finish_delay_seconds ?? 90)}" /><div class="note">${this._t("secondsShort")}</div></div>
  ...
  _renderDashboardSettings() {
    ...
    const moduleCard = (id, title, sub) => `<button type="button" class="module-card ${section === id ? "active" : ""}" data-dashboard-section="${this._esc(id)}"><b>${this._esc(title)}</b><span class="muted">${this._esc(sub)}</span></button>`;
    const titleMap = {
    ...
    return `
      <div class="settings-grid wide">
        ...
        <section class="card">
          ...
          <div class="body">${this._renderDashboardSettingsContent(dashboard, section)}</div>
        </section>
      </div>
    `;
  }
        ...
        <div class="grid">
          <section class="card">
            <div class="card-h">
              ...
              <div class="toolbar">
                <div class="seg">
                  <button id="filterTripCurrent" class="${this._tripFilter === "current" ? "active" : ""}">${this._t("currentMonth")}</button>
                  <button id="filterTripAll" class="${this._tripFilter === "all" ? "active" : ""}">${this._t("allRecords")}</button>
                </div>
      ...
      <div class="grid">
        <section class="card">
          <div class="card-h">
            ...
            <div class="toolbar">
              <div class="seg">
                <button id="filterChargeCurrent" class="${this._chargeFilter === "current" ? "active" : ""}">${this._t("currentMonth")}</button>
                <button id="filterChargeAll" class="${this._chargeFilter === "all" ? "active" : ""}">${this._t("allRecords")}</button>
              </div>
              <button id="refreshBtn" class="secondary">${this._t("refresh")}</button>
            </div>
          </div>
  ...
  _render() {
    ...
    this.shadowRoot.getElementById("tabCharge")?.addEventListener("click", () => { this._activeTab = "charge"; this._status = ""; this._error = ""; this._render(); });
    this.shadowRoot.getElementById("tabTrip")?.addEventListener("click", () => { this._activeTab = "trip"; this._status = ""; this._error = ""; this._render(); });
    this.shadowRoot.getElementById("tabSettings")?.addEventListener("click", () => { this._activeTab = "settings"; this._status = ""; this._error = ""; this._ensureStationEditor(); this._render(); });
    this.shadowRoot.querySelectorAll("tr.record").forEach((row) => row.addEventListener("click", () => {
      if (row.dataset.kind === "trip") this._selectTripRecord(row.dataset.id);
    ...
    this.shadowRoot.getElementById("filterChargeCurrent")?.addEventListener("click", () => { this._chargeFilter = "current"; this._render(); });
    this.shadowRoot.getElementById("filterChargeAll")?.addEventListener("click", () => { this._chargeFilter = "all"; this._render(); });
    this.shadowRoot.getElementById("filterTripCurrent")?.addEventListener("click", () => { this._tripFilter = "current"; this._render(); });
    this.shadowRoot.getElementById("filterTripAll")?.addEventListener("click", () => { this._tripFilter = "all"; this._render(); });
    ...
    ["general","charging","trip","ai_settings","telegram","ai","automations","dashboard"].forEach((tab) => {
      this.shadowRoot.getElementById(`settingsNav_${tab}`)?.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        this._lastClickDebug = { direct_settings_tab: tab, active_settings_tab: this._activeSettingsTab };
    ...
    this.shadowRoot.querySelectorAll("[data-dashboard-section]").forEach((el) => {
      el.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        this._lastClickDebug = { direct_dashboard_section: el.dataset.dashboardSection || "", active_dashboard_section: this._dashboardSettingsSection };
        this._switchDashboardSectionDirect(el.dataset.dashboardSection || "general");
      });
    });
...
if (!customElements.get("pom-tesla-report-panel")) {
  customElements.define("pom-tesla-report-panel-alpha139", PomTeslaReportPanel);
}
