class PomTeslaTripReportCard extends HTMLElement {
  static get version() { return '1.8.10'; }
  constructor() {
    super();
  ...
  render() {
    ...
    const avgSpeedComputed = this._computedDurationSeconds() > 0 ? this._computedDistance() / (this._computedDurationSeconds() / 3600) : 0;
    const avgSpeed = this._overrideText(c.trip_average_speed_entity, null) ?? (live ? this._formatNumber(this._attr(live, 'average_speed', 0), 0, '0') : this._formatNumber(avgSpeedComputed, 0, '0'));
    const consumptionComputed = this._computedDistance() > 0 ? usedEnergyNum / this._computedDistance() * 100 : 0;
    ...
    const batteryText = live ? String(this._attr(live, 'battery_text', '—')) : (startBattery !== null && endBattery !== null ? `%${startBattery} → %${endBattery}` : '—');
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .car { width: clamp(48px, 7cqw, 66px); height: clamp(48px, 7cqw, 66px); border-radius: 50%; background: #ff4f5b; box-shadow: 0 0 28px rgba(255,79,91,.38); display:flex; align-items:center; justify-content:center; font-size: clamp(20px, 3cqw, 29px); flex: 0 0 auto; }
        .divider { height: 1px; background: var(--pom-line); margin: clamp(14px, 1.8cqw, 22px) 0 0; }
        .metrics { display:grid; grid-template-columns: repeat(6, minmax(0, 1fr)); border-bottom: 1px solid var(--pom-line); }
