class PomTeslaTripReportCard extends HTMLElement {
  static get version() { return '1.9.16-alpha357-ai-popup'; }
  constructor() {
    super();
  ...
  _toggleAiPopup(ev) {
    ...
    this._lastToggleAt = Date.now();
    this._aiPopupOpen = !this._aiPopupOpen;
    this.render();
  }
  ...
  _liveStateObj() {
    ...
    const configuredObj = configured ? allStates[configured] : null;

    // Entity registry can rename the sensor to sensor.pom_live_trip_2, etc.
    // Build all plausible live-trip candidates, then score them. In alpha346 the dashboard is back on the stable custom element because
    // some Home Assistant sessions failed to instantiate the versioned alpha345
    // card. This scorer still prevents stale duplicate live-trip sensors from
    // winning over the one with fresh AI attrs.
    const candidates = [];
    if (configuredObj && configuredObj.state !== 'unknown' && configuredObj.state !== 'unavailable') {
      candidates.push([configured, configuredObj, 1000]);
    }
    for (const [entityId, obj] of Object.entries(allStates)) {
      if (!entityId.startsWith('sensor.')) continue;
      if (!obj || obj.state === 'unknown' || obj.state === 'unavailable') continue;
      const attrs = obj.attributes || {};
      const name = String(attrs.friendly_name || '').toLowerCase();
      const looksLive = attrs.report_type === 'live_trip' || name === 'pom live trip' || name.includes('pom live trip') || entityId.includes('pom_live_trip');
      if (!looksLive) continue;
      let score = 0;
      if (entityId === configured) score += 1000;
      if (entityId === 'sensor.pom_live_trip') score += 200;
      if (attrs.ai_live_segment_size_km !== undefined || attrs.live_ai_segment_size_km !== undefined) score += 120;
      if (attrs.ai_live_latest_segment || attrs.live_ai_latest_segment) score += 90;
      if (attrs.live_comment) score += 80;
      if (String(attrs.ai_live_comment_source || attrs.live_ai_comment_source || '') === 'panel_test') score += 60;
      if (attrs.report_type === 'live_trip') score += 40;
      candidates.push([entityId, obj, score]);
    }
  ...
  _lang(live = null) {
    const raw = String(
      this._attr(live, 'report_language', '') ||
      this._attr(live, 'language', '') ||
      this._config.report_language ||
  ...
  _t(key, lang = 'tr') {
    const en = {
      title: 'Trip Report',
      liveTripComment: 'Live Trip Comment',
      ready: 'Ready',
      waiting: 'Waiting',
      preparing: 'Preparing',
      error: 'Error',
      ...
      finishing: 'Finishing',
      final: 'Final',
      firstWaiting: 'The first Live Trip comment will be ready around {size} km.',
      nextWaiting: 'About {remaining} km left until the next Live Trip comment.',
      preparingNew: 'Preparing a new Live Trip comment.',
      thresholdPassed: 'Comment threshold reached',
      nextComment: 'Next comment',
      current: 'Current',
      lastSegment: 'Last segment',
      aiComment: 'AI comment',
      smartSummary: 'Smart summary',
      segmentPreparing: 'Comment is being prepared',
      segmentPreparingBody: 'A comment for the last {size} km segment is being prepared. It will appear here in a few seconds.',
      generatingFooter: 'Generating new segment analysis',
      temporaryErrorBody: 'The comment could not be prepared right now. It will try again on the next segment as the drive continues.',
      temporaryErrorFooter: 'Temporary error',
      distance: 'Distance',
      duration: 'Duration',
      traffic: 'Traffic',
      avgSpeed: 'Avg. speed',
      ...
      climate: 'Climate',
      elevation: 'Elevation',
      carTitle: 'Live Trip AI comment panel',
      carAria: 'Open or close the Live Trip AI comment panel',
      popupHint: 'Tap anywhere to close',
      popupTitle: 'Live Trip AI Comment',
      chipSegment: 'Segment',
      chipDistance: 'Distance',
      chipDuration: 'Duration',
      chipTraffic: 'Traffic',
      chipConsumption: 'Consumption',
    ...
    const tr = {
      ...
      finishing: 'Bitiyor',
      final: 'Final',
      firstWaiting: 'İlk Live Trip yorumu {size} km civarında hazır olacak.',
      nextWaiting: 'Bir sonraki Live Trip yorumu için yaklaşık {remaining} km kaldı.',
      preparingNew: 'Yeni Live Trip yorumu hazırlanıyor.',
      thresholdPassed: 'Yorum eşik noktası aşıldı',
      nextComment: 'Sonraki yorum',
      current: 'Şu an',
      lastSegment: 'Son bölüm',
      aiComment: 'AI yorum',
      smartSummary: 'Akıllı özet',
      segmentPreparing: 'Yorum hazırlanıyor',
      segmentPreparingBody: 'Son {size} km bölümü için yorum hazırlanıyor. Birkaç saniye içinde burada görünecek.',
      generatingFooter: 'Yeni bölüm analizi üretiliyor',
      temporaryErrorBody: 'Yorum şu anda hazırlanamadı. Sürüş ilerledikçe sonraki bölümde tekrar denenecek.',
      temporaryErrorFooter: 'Geçici hata',
      distance: 'Mesafe',
      duration: 'Süre',
      traffic: 'Trafik',
      avgSpeed: 'Ort. hız',
      overallAvg: 'Genel ort.',
      energy: 'Enerji',
      consumption: 'Tüketim',
      battery: 'Batarya',
      climate: 'Klima',
      elevation: 'Rakım',
      carTitle: 'Live Trip AI yorum paneli',
      carAria: 'Live Trip AI yorum panelini aç veya kapat',
      popupHint: 'Kapatmak için ekrana dokun',
      popupTitle: 'Live Trip AI Yorumu',
      chipSegment: 'Bölüm',
      chipDistance: 'Mesafe',
      chipDuration: 'Süre',
      chipTraffic: 'Trafik',
      chipConsumption: 'Tüketim',
      chipElevation: 'Rakım',
      chipClimate: 'Klima',
      chipLocation: 'Konum',
      chipSpeed: 'Ort. hız',
  ...
  _tpl(text, values = {}) {
    return String(text || '').replace(/\{(\w+)\}/g, (_m, key) => values[key] ?? '');
  }

  _localizedTitle(title, lang) {
    const raw = String(title || '').trim();
    if (!raw || raw === 'Sürüş Raporu' || raw === 'Trip Report') return this._t('title', lang);
    return raw;
  }

  _localizeStatusValue(value, lang) {
    const raw = String(value || '').trim().toLowerCase();
    if (raw === 'generating') return this._t('preparing', lang);
    if (raw === 'ready') return this._t('ready', lang);
    if (raw === 'error') return this._t('error', lang);
    if (raw === 'waiting') return this._t('waiting', lang);
    if (raw === 'active' || raw === 'canlı') return this._t('live', lang);
    if (raw === 'finishing' || raw === 'bitiyor') return this._t('finishing', lang);
    if (raw === 'finished' || raw === 'final') return this._t('final', lang);
    if (raw === 'idle' || raw === 'hazır') return this._t('ready', lang);
    return value || this._t('ready', lang);
  }

  _localizeAiText(text, lang) {
    let t = String(text || '').trim();
    if (lang !== 'en' || !t) return t;
    t = t.replace(/Yeni Live Trip yorumu hazırlanıyor\.?/gi, 'Preparing a new Live Trip comment.');
    t = t.replace(/İlk Live Trip yorumu\s+([0-9]+(?:[.,][0-9]+)?)\s+km civarında hazır olacak\.?/gi, (_m, n) => `The first Live Trip comment will be ready around ${String(n).replace(',', '.')} km.`);
    t = t.replace(/Bir sonraki Live Trip yorumu için yaklaşık\s+([0-9]+(?:[.,][0-9]+)?)\s+km kaldı\.?(?:\s*Hedef:\s*([0-9]+(?:[.,][0-9]+)?)\s*km\.)?/gi, (_m, n, target) => {
      const base = `About ${String(n).replace(',', '.')} km left until the next Live Trip comment.`;
      return target ? `${base} Target: ${String(target).replace(',', '.')} km.` : base;
    });
    t = t.replace(/Yorum şu anda hazırlanamadı\. Sürüş ilerledikçe sonraki bölümde tekrar denenecek\.?/gi, this._t('temporaryErrorBody', 'en'));
    t = t.replace(/(.+?) için yorum hazırlanıyor\.\.\./gi, (_m, label) => `Preparing a comment for ${label}...`);
    return t;
  }
  ...
  _durationText(seconds, lang = this._lang()) {
  ...
  _compactDurationText(value, lang = this._lang()) {
    if (value === null || value === undefined) return '—';
    let text = String(value).trim();
    if (!text || text === '—') return '—';

    const lower = text.toLowerCase();
    if (lower.includes('kullanılmadı') || lower.includes('not used')) return '—';
    ...
    if (lang === 'en') {
      text = text
        .replace(/(\d+)\s*sa\s+(\d+)\s*dk/gi, '$1h $2min')
        .replace(/(\d+)\s*sa/gi, '$1h')
        .replace(/(\d+)\s*dk/gi, '$1min')
        .replace(/dakika/gi, 'min')
        .replace(/saat/gi, 'h')
        .replace(/\s+/g, ' ')
        .trim();
    }
  ...
  _liveAiPanelData(live, distanceText, lang = this._lang(live)) {
    ...
    const segments = this._liveAiSegments(live);
    const rawSegmentSize = Number(this._liveAiAttr(live, 'ai_live_segment_size_km', 'live_ai_segment_size_km', 10));
    const segmentSize = Number.isFinite(rawSegmentSize) && rawSegmentSize > 0 ? rawSegmentSize : 10;
    const rawCompletedIndex = Number(this._liveAiAttr(live, 'ai_live_last_completed_index', 'live_ai_segment_last_completed_index', 0) || 0) || 0;
    const latestIsPanelTest = Boolean(latest?.test_mode) || String(latest?.source || '').trim() === 'panel_test';
    const distanceBasedCompletedIndex = tripKm > 0 && segmentSize > 0 ? Math.floor(Math.max(0, tripKm - 0.0001) / segmentSize) : 0;
    const lastCompletedIndex = latestIsPanelTest
      ? Math.max(0, rawCompletedIndex)
      : Math.max(rawCompletedIndex, Number(latest?.segment_index || 0) || 0, distanceBasedCompletedIndex);
    const derivedNextTarget = tripKm > 0.05
      ? Math.max(segmentSize, (Math.floor(tripKm / segmentSize) + 1) * segmentSize)
      : Math.max(segmentSize, (Math.max(0, rawCompletedIndex) + 1) * segmentSize);
    const rawNextTarget = Number(this._liveAiAttr(live, 'ai_live_next_target_km', 'live_ai_segment_next_target_km', derivedNextTarget));
    const rawNextIsUsable = Number.isFinite(rawNextTarget) && rawNextTarget > Math.max(0, tripKm - 0.05);
    const nextTarget = rawNextIsUsable && Math.abs(rawNextTarget - derivedNextTarget) <= Math.max(0.05, segmentSize * 0.35)
      ? rawNextTarget
      : derivedNextTarget;
    ...
    const computedWaitingText = tripKm <= 0.05
      ? this._tpl(this._t('firstWaiting', lang), { size: segmentLabel })
      : this._tpl(this._t('nextWaiting', lang), { remaining: remainingKm.toFixed(1) });
    const rawDistanceMatch = rawWaitingText.match(/yaklaşık\s+([0-9]+(?:[.,][0-9]+)?)/i);
    const rawDistance = rawDistanceMatch ? Number(String(rawDistanceMatch[1]).replace(',', '.')) : NaN;
    const rawLooksStale = rawWaitingText && (
      (Number.isFinite(rawDistance) && Math.abs(rawDistance - remainingKm) > Math.max(0.35, segmentSize * 0.25)) ||
      (lang === 'en' && /[ıİğĞşŞçÇöÖüÜ]|Bir sonraki|İlk Live Trip|Hazırlanıyor|yorum/i.test(rawWaitingText)) ||
      (segmentSize !== 10 && rawWaitingText.includes('10 km')) ||
      (tripKm <= 0.05 && !rawWaitingText.includes(segmentLabel))
    );
    const waitingText = rawWaitingText && !rawLooksStale ? rawWaitingText : computedWaitingText;
    const segmentStart = Math.max(0, Math.floor(Math.max(0, tripKm - 0.0001) / segmentSize) * segmentSize);
    ...
    const latestComment = this._localizeAiText(String(latest?.comment || this._attr(live, 'live_comment', '') || '').trim(), lang);
    let title = '';
    let body = latestComment;
    let footer = latest ? `${latest.segment_label || this._t('lastSegment', lang)} · ${latest.source === 'openai' ? this._t('aiComment', lang) : this._t('smartSummary', lang)}` : waitingText;
    if (status === 'generating') {
      title = latest?.segment_label ? (lang === 'en' ? `${latest.segment_label} preparing` : `${latest.segment_label} hazırlanıyor`) : this._t('segmentPreparing', lang);
      body = latestComment || this._tpl(this._t('segmentPreparingBody', lang), { size: segmentLabel });
      footer = this._t('generatingFooter', lang);
    } else if (status === 'error') {
      title = '';
      body = this._t('temporaryErrorBody', lang);
      footer = this._t('temporaryErrorFooter', lang);
    } else if (!body) {
      title = '';
      body = waitingText;
      footer = `${this._t('current', lang)}: ${distanceText} km`;
    }
    return { tripKm, status, source, waitingText, latest, segments, nextTarget, segmentSize, progress, remainingKm, title, body, footer };
  }


  _renderAiPopupChips(aiPanel, live, lang) {
    const latest = aiPanel?.latest && typeof aiPanel.latest === 'object' ? aiPanel.latest : {};
    const chips = [];
    const add = (labelKey, value) => {
      const text = String(value ?? '').trim();
      if (!text || text === '—' || text.toLowerCase() === 'nan') return;
      chips.push(`<span class="ai-popup-chip"><b>${this._escapeHtml(this._t(labelKey, lang))}</b>${this._escapeHtml(text)}</span>`);
    };
    const num = (value) => {
      const n = Number(value);
      return Number.isFinite(n) ? n : null;
    };
    add('chipSegment', latest.segment_label || '');
    const latestDistance = num(latest.distance_km);
    if (latestDistance !== null && latestDistance > 0) add('chipDistance', `${latestDistance.toFixed(latestDistance < 10 ? 1 : 0)} km`);
    add('chipDuration', latest.duration_text || this._attr(live, 'duration_text', ''));
    add('chipTraffic', latest.traffic_text || this._attr(live, 'traffic_text', ''));
    const consumption = num(latest.consumption_kwh_100km ?? this._attr(live, 'consumption_kwh_100km', null));
    if (consumption !== null && consumption > 0) add('chipConsumption', `${consumption.toFixed(1)} kWh/100`);
    const location = latest.current_road || latest.current_location_label || latest.current_neighbourhood || latest.current_dashboard_location_label || this._attr(live, 'current_road', '') || this._attr(live, 'current_location_label', '') || this._attr(live, 'location_label', '');
    add('chipLocation', location);
    const minElev = num(latest.min_elevation ?? this._attr(live, 'min_elevation', null));
    const maxElev = num(latest.max_elevation ?? this._attr(live, 'max_elevation', null));
    const elevDelta = num(latest.elevation_delta ?? this._attr(live, 'elevation_delta', null));
    const elevRange = num(latest.elevation_range ?? this._attr(live, 'elevation_range', null));
    if (minElev !== null && maxElev !== null && (minElev !== 0 || maxElev !== 0)) {
      add('chipElevation', `${Math.round(minElev)}–${Math.round(maxElev)} m`);
    } else if (elevDelta !== null && Math.abs(elevDelta) >= 1) {
      add('chipElevation', `${elevDelta > 0 ? '+' : ''}${Math.round(elevDelta)} m`);
    } else if (elevRange !== null && elevRange > 0) {
      add('chipElevation', `${Math.round(elevRange)} m range`);
    }
    add('chipClimate', latest.climate_text || this._attr(live, 'climate_text', ''));
    const avg = num(latest.average_speed ?? this._attr(live, 'average_speed', null));
    if (avg !== null && avg > 0) add('chipSpeed', `${Math.round(avg)} ${this._t('speedUnit', lang)}`);
  ...
  render() {
    ...
    const lang = this._lang(live);
    const titleText = this._localizedTitle(c.title, lang);
    ...
    const duration = this._compactDurationText(this._overrideText(c.trip_duration_entity, null) ?? (live ? String(this._attr(live, 'duration_text', '—')) : this._durationText(this._computedDurationSeconds(), lang)), lang);
    const traffic = this._compactDurationText(this._overrideText(c.trip_traffic_entity, null) ?? (live ? String(this._attr(live, 'traffic_text', '—')) : this._durationText(s.traffic_seconds || 0, lang)), lang);
    ...
    const climateDuration = this._compactDurationText(this._overrideText(c.trip_climate_entity, null) ?? (live ? String(this._attr(live, 'climate_text', '—')) : this._durationText(s.climate_seconds || 0, lang)), lang);
    ...
    const status = live ? this._localizeStatusValue(this._attr(live, 'status', this._t('ready', lang)), lang) : this._statusText(lang);
    const aiPanel = this._liveAiPanelData(live, distance, lang);
    const aiStatusLabel = aiPanel.status === 'generating' ? this._t('preparing', lang) : (aiPanel.status === 'ready' ? this._t('ready', lang) : (aiPanel.status === 'error' ? this._t('error', lang) : this._t('waiting', lang)));
    const aiHistory = this._renderAiHistoryChips(aiPanel.segments, aiPanel.latest?.segment_index);
    const aiPopupChips = this._renderAiPopupChips(aiPanel, live, lang);
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .ai-panel.open { max-height: 450px; opacity: 1; transform: translateY(0); pointer-events: auto; }
        ...
        .ai-comment-shell { cursor: pointer; }
        .ai-comment-shell:hover { border-color: rgba(96,165,250,.34); box-shadow: inset 0 1px 0 rgba(255,255,255,.06), 0 16px 42px rgba(96,165,250,.12); }
        .ai-comment-shell:active { transform: translateY(1px) scale(.998); }
        .ai-popup-overlay {
          position: fixed;
          inset: 0;
          z-index: 999999;
          ...
          background:
            radial-gradient(circle at 28% 22%, rgba(96,165,250,.24), transparent 34%),
            radial-gradient(circle at 78% 28%, rgba(236,72,153,.22), transparent 36%),
            rgba(3, 6, 18, .68);
        ...
        .ai-popup-card {
          ...
          border: 1px solid rgba(121,128,255,.34);
          background:
            ...
            linear-gradient(145deg, rgba(13,15,28,.96), rgba(23,16,31,.95));
          box-shadow: 0 40px 110px rgba(0,0,0,.58), inset 0 1px 0 rgba(255,255,255,.06);
        ...
        @media (max-width: 900px) {
          .ai-popup-card { width: 96vw; padding: 24px; }
          .ai-popup-body { font-size: clamp(24px, 4.8vw, 40px); }
          .ai-popup-chip { font-size: 13px; padding: 9px 12px; }
        }
        @keyframes aiPulse {
      ...
      <ha-card>
        <div class="wrap">
          <div class="top">
            ...
            <button class="car ${this._commentPanelOpen ? 'open' : ''}" type="button" title="${this._t('carTitle', lang)}" aria-label="${this._t('carAria', lang)}">${c.car_icon}<span class="ai-mini-badge">AI</span></button>
          </div>
          <section class="ai-panel ${this._commentPanelOpen ? 'open' : ''}">
            <div class="ai-panel-inner">
              <div class="ai-panel-head">
                <div>
                  <div class="ai-kicker">${this._t('liveTripComment', lang)}</div>
                </div>
              ...
              <div class="ai-progress-wrap">
                <div class="ai-progress-head">
                  ...
                  <span>${this._escapeHtml(aiPanel.remainingKm > 0.01 ? `${this._t('nextComment', lang)} ~${(Math.abs(aiPanel.nextTarget - Math.round(aiPanel.nextTarget)) < 0.05 ? aiPanel.nextTarget.toFixed(0) : aiPanel.nextTarget.toFixed(1))} km` : this._t('thresholdPassed', lang))}</span>
                </div>
          ...
          <div class="metrics">
            <div class="metric"><div class="label">${this._t('distance', lang)}</div><div class="metric-value"><span class="value">${distance}</span><span class="unit">km</span></div></div>
            <div class="metric"><div class="label">${this._t('duration', lang)}</div><div class="metric-value"><span class="value">${duration}</span></div></div>
            <div class="metric"><div class="label">${this._t('traffic', lang)}</div><div class="metric-value"><span class="value">${traffic}</span></div></div>
            <div class="metric"><div class="label">${this._t('avgSpeed', lang)}</div><div class="metric-value"><span class="value">${avgSpeed}</span><span class="unit">${this._t('speedUnit', lang)}</span></div></div>
            <div class="metric"><div class="label">${this._t('overallAvg', lang)}</div><div class="metric-value"><span class="value">${overallSpeed}</span><span class="unit">${this._t('speedUnit', lang)}</span></div></div>
            <div class="metric"><div class="label">${this._t('energy', lang)}</div><div class="metric-value"><span class="value red">${tripEnergy}</span><span class="unit">kWh</span></div></div>
            <div class="metric"><div class="label">${this._t('consumption', lang)}</div><div class="metric-value"><span class="value">${consumption}</span><span class="unit">kWh/100</span></div></div>
          </div>
          <div class="bottom">
            <div class="box">
              <div class="box-label">▰ ${this._t('battery', lang)}</div>
            ...
            <div class="box"><div class="box-label">❄️ ${this._t('climate', lang)}</div><div class="box-value">${climateDuration}</div></div>
            <div class="box"><div class="box-label">⛰️ ${this._t('elevation', lang)}</div><div class="box-value">${elevationText}</div></div>
          </div>
        </div>
      </ha-card>
      ${this._aiPopupOpen ? `
        <div class="ai-popup-overlay" role="dialog" aria-modal="true">
          <div class="ai-popup-card">
            <div class="ai-popup-kicker">${this._escapeHtml(this._t('popupTitle', lang))}</div>
            <div class="ai-popup-body">${this._escapeHtml(aiPanel.body)}</div>
            ${aiPopupChips ? `<div class="ai-popup-chips">${aiPopupChips}</div>` : ``}
            <div class="ai-popup-hint">${this._escapeHtml(this._t('popupHint', lang))}</div>
          </div>
        </div>` : ``}
    `;
    ...
    const commentShell = this.shadowRoot.querySelector('.ai-comment-shell');
    if (commentShell) {
      commentShell.onclick = (ev) => this._toggleAiPopup(ev);
    }
    const popupOverlay = this.shadowRoot.querySelector('.ai-popup-overlay');
    if (popupOverlay) {
      popupOverlay.onclick = (ev) => this._closeAiPopup(ev);
    }
  }
...
if (!customElements.get('pom-tesla-trip-report-card')) {
  customElements.define('pom-tesla-trip-report-card', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha339')) {
  customElements.define('pom-tesla-trip-report-card-alpha339', PomTeslaTripReportCard);
}
...
if (!customElements.get('pom-tesla-trip-report-card-alpha342')) {
  customElements.define('pom-tesla-trip-report-card-alpha342', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha343')) {
  customElements.define('pom-tesla-trip-report-card-alpha343', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha344')) {
  customElements.define('pom-tesla-trip-report-card-alpha344', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha345')) {
  customElements.define('pom-tesla-trip-report-card-alpha345', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha346')) {
  customElements.define('pom-tesla-trip-report-card-alpha346', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha347')) {
  customElements.define('pom-tesla-trip-report-card-alpha347', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha348')) {
  customElements.define('pom-tesla-trip-report-card-alpha348', PomTeslaTripReportCard);
}
if (!customElements.get('pom-tesla-trip-report-card-alpha357')) {
  customElements.define('pom-tesla-trip-report-card-alpha357', PomTeslaTripReportCard);
}
...
window.customCards.push({
  type: 'pom-tesla-trip-report-card',
