class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha223-source-locked-master-autofind";
...
.ai-role-meta-line { color:#fca5a5; }
  ...
  _renderSourceBadge(entry) {
    if (!entry || !entry.entity_id) return "";
    const label = String(entry.source_label || entry.source_key || entry.source_platform || "").trim();
    if (!label) return "";
    const key = String(entry.source_key || label).toLowerCase().replace(/[^a-z0-9_\-]/g, "-");
...
if (!customElements.get("pom-tesla-report-panel")) {
  customElements.define("pom-tesla-report-panel-alpha223", PomTeslaReportPanel);
}
