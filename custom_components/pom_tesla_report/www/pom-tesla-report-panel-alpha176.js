class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha176-report-cost-slots";
  ...
  _filterLabel(kind) {
    const filter = kind === "trip" ? this._tripFilter : this._chargeFilter;
    const month = kind === "trip" ? this._tripSelectedMonth : this._chargeSelectedMonth;
  ...
  _filterRecordsByDate(records, kind) {
    ...
    const selectedMonth = kind === "trip" ? this._tripSelectedMonth : this._chargeSelectedMonth;
    const rangeStart = kind === "trip" ? this._tripRangeStart : this._chargeRangeStart;
    const rangeEnd = kind === "trip" ? this._tripRangeEnd : this._chargeRangeEnd;
  ...
  _applyDateRange(kind) {
    const root = this.shadowRoot;
    const prefix = kind === "trip" ? "Trip" : "Charge";
  ...
  _styles() {
    return `
      ...
      .station-row-actions { display:flex; align-items:center; justify-content:flex-end; gap:8px; flex-wrap:wrap; }
      .mini-slot-btn { width:32px; min-width:32px; height:32px; padding:0; border-radius:10px; font-size:12px; background:rgba(15,23,42,.88); border:1px solid rgba(56,189,248,.28); color:#bae6fd; }
      .mini-slot-btn:hover { border-color:#38bdf8; color:#fff; }
      .report-slot-badge { padding:6px 9px; border-radius:999px; border:1px solid rgba(45,212,191,.25); background:rgba(20,184,166,.10); color:#99f6e4; font-size:11px; font-weight:800; white-space:nowrap; }
  ...
  _renderDateFilterToolbar(kind) {
    ...
    const prefix = isTrip ? "Trip" : "Charge";
    const filter = isTrip ? this._tripFilter : this._chargeFilter;
    const selectedMonth = isTrip ? this._tripSelectedMonth : this._chargeSelectedMonth;
    const rangeStart = isTrip ? this._tripRangeStart : this._chargeRangeStart;
    const rangeEnd = isTrip ? this._tripRangeEnd : this._chargeRangeEnd;
    const panelOpen = isTrip ? this._tripDatePanelOpen : this._chargeDatePanelOpen;
    const records = isTrip ? (this._tripData?.records || []) : (this._chargeData?.records || []);
    ...
    const stationRow = (s, index, mode = "pool") => `
      <div class="station-row ${index === this._selectedStationIndex ? "selected" : ""} ${index < 3 ? "report-active" : ""}" data-station-index="${index}">
        ...
        <div class="station-row-actions">
          ${mode === "pool" ? `
            <button class="mini-slot-btn" data-station-slot="0" data-station-index="${index}" title="${this._esc(this._t("moveToReportSlot"))} 1">1</button>
            <button class="mini-slot-btn" data-station-slot="1" data-station-index="${index}" title="${this._esc(this._t("moveToReportSlot"))} 2">2</button>
            <button class="mini-slot-btn" data-station-slot="2" data-station-index="${index}" title="${this._esc(this._t("moveToReportSlot"))} 3">3</button>
          ` : `<span class="report-slot-badge">${this._t("reportSlot")} ${index + 1}</span>`}
...
if (!customElements.get("pom-tesla-report-panel")) {
  customElements.define("pom-tesla-report-panel-alpha176", PomTeslaReportPanel);
}
