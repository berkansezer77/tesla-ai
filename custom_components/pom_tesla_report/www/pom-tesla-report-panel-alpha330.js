class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha331-backup-ui-wait-modal";
  ...
  _t(key) {
    const dict = {
      tr: {
        ...
        backupSimpleSub: "En güvenlisi tam yedek almaktır. Sadece ayar veya kayıt dosyası gerekiyorsa aşağıdaki küçük seçenekleri kullan.",
      ...
      en: {
        ...
        backupSimpleSub: "The safest option is a full backup. Use the smaller options only when you need a settings or records file.",
  ...
  _styles() {
    return `
      ...
      .backup-busy-copy small { color:#fecaca; font-weight:800; }
...
.ai-role-meta-line { color:#fca5a5; }
      ...
      <div class="backup-busy-overlay" role="alertdialog" aria-live="assertive" aria-modal="true">
        <div class="backup-busy-modal">
          ...
          <div class="backup-busy-copy">
            <b>${this._esc(title)}</b>
            <p>${this._esc(sub)}</p>
            <small>${this._t("backupBusyFooter")}</small>
          </div>
        </div>
      </div>
    `;
        ...
        <section class="card backup-card">
          ...
          <div class="body">
            ...
            <div class="backup-footer-row">
              ...
              <button id="exportFullBackupJsonBtn" class="secondary tiny backup-json-advanced" type="button">${this._t("exportFullBackupJson")}</button>
            </div>
  ...
  async _exportRecordsBackup() {
    ...
    this._backupBusyTitle = this._t("backupBusyTitle");
    this._backupBusySub = this._t("backupBusySub");
    this._loading = false;
    this._error = "";
    this._render();
    try {
...
if (!customElements.get("pom-tesla-report-panel-alpha330")) {
  customElements.define("pom-tesla-report-panel-alpha330", PomTeslaReportPanel);
}
