class PomTeslaReportPanel extends HTMLElement {
  constructor() {
    ...
    this._frontendBuild = "alpha238-ai-grade-report-png-layout";
  ...
  _styles() {
    return `
      ...
      .health-head { display:flex; justify-content:space-between; gap:14px; align-items:flex-start; margin-bottom:14px; }
      ...
      .health-grid > div { background:rgba(15,23,42,.68); border:1px solid rgba(148,163,184,.16); border-radius:16px; padding:12px; min-height:74px; }
      .health-grid span { display:block; color:#94a3b8; font-size:11px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; }
      .health-grid b { display:block; margin-top:7px; color:#e5edf6; font-size:17px; word-break:break-word; }
      .health-grid small { display:block; margin-top:4px; color:#94a3b8; }
...
.ai-role-meta-line { color:#fca5a5; }
  ...
  _renderSystemHealthCard() {
    const health = this._settingsData?.general_settings?.health || {};
    ...
    return `
      <section class="${cls}">
        ...
        <div class="health-grid">
          <div><span>${this._t("healthMemory")}</span><b>${this._esc(String(rss || "-"))} MB${total ? ` / ${this._esc(String(total))} MB` : ""}</b><small>${pct ? `%${this._esc(String(pct))}` : ""}</small></div>
          <div><span>${this._t("healthAvailable")}</span><b>${available ? `${this._esc(String(available))} MB` : "-"}</b><small>${memory.available_percent ? `%${this._esc(String(memory.available_percent))}` : ""}</small></div>
          ...
          <div><span>${this._t("healthEntities")}</span><b>${this._esc(String(counts.ha_entities ?? "-"))}</b><small>HA</small></div>
          <div><span>${this._t("healthPomEntities")}</span><b>${this._esc(String(counts.pom_entities ?? "-"))}</b><small>POM</small></div>
  ...
  _render() {
    ...
    this.shadowRoot.getElementById("refreshBtnHealth")?.addEventListener("click", () => this._loadActive());
...
if (!customElements.get("pom-tesla-report-panel")) {
  customElements.define("pom-tesla-report-panel-alpha238", PomTeslaReportPanel);
}
