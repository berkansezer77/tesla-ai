class PomTeslaTripReportCard extends HTMLElement {
  static get version() { return '1.9.1-alpha326'; }
  constructor() {
    super();
  ...
  _liveAiSegments(live) {
    const raw = live?.ai_live_segments;
    return Array.isArray(raw) ? raw.filter(Boolean) : [];
  }

  _liveAiLatestSegment(live) {
    const latest = live?.ai_live_latest_segment;
  ...
  _liveAiPanelData(live, distanceText) {
    ...
    const status = String(this._attr(live, 'ai_live_comment_status', 'waiting') || 'waiting').trim().toLowerCase();
    const source = String(this._attr(live, 'ai_live_comment_source', '') || '').trim();
    const waitingText = String(this._attr(live, 'ai_live_waiting_text', '') || '').trim();
    const latest = this._liveAiLatestSegment(live);
    const segments = this._liveAiSegments(live);
    const nextTarget = Number(this._attr(live, 'ai_live_next_target_km', 10)) || 10;
    const segmentSize = Number(this._attr(live, 'ai_live_segment_size_km', 10)) || 10;
    const lastCompletedIndex = Number(latest?.segment_index || 0) || 0;
    ...
    if (status === 'generating') {
      ...
      body = latestComment || 'Son 10 km bölümü için yorum hazırlanıyor. Birkaç saniye içinde burada görünecek.';
      ...
      title = 'Live Trip AI';
      body = waitingText || 'İlk yorum 10 km civarında hazır olacak.';
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .ai-panel.open { max-height: 440px; opacity: 1; transform: translateY(0); pointer-events: auto; }
        ...
        .ai-title { margin-top: 6px; font-size: clamp(18px, 1.8cqw, 28px); font-weight: 950; line-height: 1.08; color: #fff; }
        ...
        .ai-meta-label { font-size: 11px; font-weight: 900; color: rgba(255,255,255,.5); letter-spacing: .08em; text-transform: uppercase; }
        .ai-meta-value { margin-top: 6px; font-size: clamp(14px, 1.2cqw, 18px); font-weight: 900; color: #fff; }
            ...
            <div class="ai-panel-inner">
              <div class="ai-panel-head">
                <div>
                  ...
                  <div class="ai-title">${this._escapeHtml(aiPanel.title)}</div>
                </div>
                <div class="ai-status-badge ${aiPanel.status}"><span class="ai-status-dot"></span>${aiStatusLabel}</div>
              </div>
              <div class="ai-body">${this._escapeHtml(aiPanel.body)}</div>
              <div class="ai-meta-grid">
                <div class="ai-meta">
                  <div class="ai-meta-label">Bölüm</div>
                  <div class="ai-meta-value">${this._escapeHtml(aiPanel.latest?.segment_label || 'İlk yorum bekleniyor')}</div>
                </div>
                <div class="ai-meta">
                  <div class="ai-meta-label">Şu anki mesafe</div>
                  <div class="ai-meta-value">${this._escapeHtml(`${distance} km`)}</div>
                </div>
                <div class="ai-meta">
                  <div class="ai-meta-label">Kaynak</div>
                  <div class="ai-meta-value">${this._escapeHtml(aiPanel.source === 'openai' ? 'OpenAI' : (aiPanel.source ? 'Akıllı özet' : 'Bekleniyor'))}</div>
                </div>
              </div>
  ...
  static getStubConfig() {
    return {
      type: 'custom:pom-tesla-trip-report-card-alpha326',
      live_trip_entity: '',
      speed_entity: '',
      odometer_entity: '',
      energy_remaining_entity: '',
...
if (!customElements.get('pom-tesla-trip-report-card-alpha326')) {
  customElements.define('pom-tesla-trip-report-card-alpha326', PomTeslaTripReportCard);
}
...
window.customCards.push({
  type: 'pom-tesla-trip-report-card-alpha326',
  name: 'POM Tesla Trip Report Card alpha326',
