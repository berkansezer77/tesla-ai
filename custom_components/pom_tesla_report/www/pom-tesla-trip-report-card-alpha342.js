class PomTeslaTripReportCard extends HTMLElement {
  static get version() { return '1.9.9-alpha343'; }
  constructor() {
    super();
  ...
  _liveAiPanelData(live, distanceText) {
    ...
    const lastCompletedIndex = latestIsPanelTest
      ? rawCompletedIndex
      : (Number(latest?.segment_index || rawCompletedIndex || 0) || 0);
    const derivedNextTarget = Math.max(segmentSize, (Math.max(0, rawCompletedIndex) + 1) * segmentSize);
    const rawNextTarget = Number(this._liveAiAttr(live, 'ai_live_next_target_km', 'live_ai_segment_next_target_km', derivedNextTarget));
    const nextTarget = Number.isFinite(rawNextTarget) && rawNextTarget > 0 && Math.abs(rawNextTarget - derivedNextTarget) <= Math.max(0.05, segmentSize * 0.25)
      ? rawNextTarget
      : derivedNextTarget;
    ...
    const rawLooksStale = rawWaitingText && Math.abs((Number((rawWaitingText.match(/yaklaşık\s+([0-9]+(?:[.,][0-9]+)?)/i) || [])[1]?.replace(',', '.')) || -999) - Math.max(0, nextTarget - tripKm)) > 0.6;
    const waitingText = rawWaitingText && !rawLooksStale && !(segmentSize !== 10 && rawWaitingText.includes('10 km')) ? rawWaitingText : computedWaitingText;
...
window.customCards = window.customCards || [];
if (!window.customCards.some((card) => card && card.type === 'pom-tesla-trip-report-card-alpha342')) {
  window.customCards.push({
    type: 'pom-tesla-trip-report-card-alpha342',
    name: 'POM Tesla Trip Report Card alpha342',
    description: 'Live Tesla trip report card alpha342 bridge for POM Tesla Report.',
  });
}
