...
def _draw_energy_panel(draw: ImageDraw.ImageDraw, data: dict[str, Any], top: int) -> int:
    ...
    cx = WIDTH // 2
    draw.text((cx - 118, top + 38), "EKLENEN ENERJİ", font=_font(18, True), fill=BLUE)
...
def _draw_costs(draw: ImageDraw.ImageDraw, data: dict[str, Any], top: int) -> int:
    ...
    rows = [
        ("ZES", float(data.get("zes_price") or data.get("zes_kwh_price") or 0.0), GREEN),
        ("Supercharger", float(data.get("supercharger_price") or data.get("supercharger_kwh_price") or 0.0), RED),
        ("Astor", float(data.get("astor_price") or data.get("astor_kwh_price") or 0.0), ORANGE),
    ]
